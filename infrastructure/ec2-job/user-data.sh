...
# Variáveis substituídas via templatefile():
...
curl -L "https://github.com/protomaps/go-pmtiles/releases/download/$PMTILES_VERSION/go-pmtiles_${PMTILES_VERSION#v}_Linux_x86_64.tar.gz" \
...
aws sns publish \
    ...
    --message "Pipeline ${STATUS} on $INSTANCE_ID. See /var/log/alfaero-pipeline.log on the instance (if still alive)." \
    || true

# ----- self-destruct (spot termina sozinho mas garante limpeza) -----
sleep 30
